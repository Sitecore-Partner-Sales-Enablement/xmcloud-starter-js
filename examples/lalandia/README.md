# Lalandia - Sitecore AI Head Application (Next.js)
...
**Lalandia** is a Sitecore AI head application cloned from `kit-nextjs-skate-park` and restyled to approximate [lalandia.dk](https://www.lalandia.dk/en) as a visual starting point (brand lime `#B9DF23`, yellow Book CTA, aqua waterpark accents, Roboto typography).

Use this example on a fork or private template copy — it is not intended as an upstream contribution of a new public starter.

## Developer Expectations

* Tailwind-based styling with Lalandia design tokens in `src/assets/base/variables.css` and `tailwind.config.js`
* Modular components for reuse (from skate-park baseline)
* Localization support for English (en) and Canadian English (en-CA)
* AI-facing routes (`llms.txt`, FAQ, summary) branded for Lalandia
...
6. Provide Editing host name  ```lalandia``` as per xmcloud.build.json
